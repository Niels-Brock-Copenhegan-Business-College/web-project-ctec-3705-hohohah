...
<div class="admin-content">
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;flex-wrap:wrap;gap:1rem;">
    ...
    <p style="color:var(--text-muted);">
      <?= count($modules) ?> module<?= count($modules) !== 1 ? 's' : '' ?> total
    </p>
      ...
      <table aria-label="Modules management table">
        ...
        <?php foreach ($modules as $m): ?>
          <tr>
            ...
            <td>
              <?= htmlspecialchars($m['leader_name'] ?? '—') ?>
            </td>

            <td>
              <div class="actions-cell">

                <a href="<?= url('/admin/modules/' . $m['id'] . '/edit') ?>"
                   class="btn btn-sm btn-primary"
                   aria-label="Edit <?= htmlspecialchars($m['title']) ?>">
                   Edit
                </a>

                <form method="POST"
                      action="<?= url('/admin/modules/' . $m['id'] . '/delete') ?>"
                      style="display:inline;"
                      onsubmit="return confirm('Delete \'<?= addslashes($m['title']) ?>\'? This cannot be undone.');">

                  <button type="submit"
                          class="btn btn-sm btn-danger"
                          aria-label="Delete <?= htmlspecialchars($m['title']) ?>">
                          Delete
                  </button>
